...
#define out cout
#else
std::ifstream input("D:/coding/crypto/dump.bin", std::ios::binary);
...
void PrintDataLE(uint8_t *data, size_t length)
{
...
// 实现PKCS7填充
void PKCS7Padding(uint8_t *data, uint32_t length)
{
...
void PKCS7Unpadding(uint8_t *data, uint32_t &length)
{
    uint8_t padding = data[length - 1]; // 获取填充字节的值
    if (padding > 0 && padding <= 16)
    {
        ...
        if (validPadding)
        {
            length -= padding; // 去掉填充字节
        }
    }
}

unsigned char prodTable[7][256] = {
...
{
    uint8_t bytes[4];           // 将 32 位 word 视为 4 个字节
    for (int i = 0; i < 4; i++) // 遍历每个字节
    {
        bytes[i] = (word >> (24 - i * 8)) & 0xFF;
        uint8_t byte = bytes[i];
        uint8_t row = (byte >> 4) & 0x0F; // 高4位作为行坐标
        uint8_t col = byte & 0x0F;        // 低4位作为列坐标
        bytes[i] = s_box[row * 16 + col]; // 查找 S 盒替换后的值
    }
...
// AES具体加密实现
void Cipher(uint8_t *data, uint32_t *key)
{
    // 初始化状态矩阵
    uint8_t state[4][4];
    ...
    for (int i = 0; i < true_len; i += 16)
    {
        ...
        if (i == true_len - 16)
        {
            ...
            else
            {
                ...
                // for (int i = 0; i < 16; i++)
                // {
                //     printf("%02x", tmp[i]); // Print each byte as 2-digit hex
                //     if ((i + 1) % 4 == 0)   // Print newline every 4 bytes
                //         printf("\n");
                //     else
                //         printf(" "); // Print space between bytes in a row
                // }
...
void InvSubBytes(uint8_t state[4][4])
{
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 4; j++)
        {
            ...
            uint8_t row = (byte >> 4) & 0x0F;        // 高4位作为行坐标
            uint8_t col = byte & 0x0F;               // 低4位作为列坐标
            state[i][j] = inv_s_box[16 * row + col]; // 在S盒中查找替换
        }
    }
}
...
void InvMixColumns(uint8_t state[4][4])
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        c = state[2][i];
        d = state[3][i];
        state[0][i] = prodTable[6][a] ^ prodTable[4][b] ^ prodTable[5][c] ^ prodTable[3][d];
        state[1][i] = prodTable[3][a] ^ prodTable[6][b] ^ prodTable[4][c] ^ prodTable[5][d];
        state[2][i] = prodTable[5][a] ^ prodTable[3][b] ^ prodTable[6][c] ^ prodTable[4][d];
        state[3][i] = prodTable[4][a] ^ prodTable[5][b] ^ prodTable[3][c] ^ prodTable[6][d];
    }
}
...
//     uint8_t *S_IV = new uint8_t[BLOCKSIZE];
//     uint8_t tmp[BLOCKSIZE];
//     uint8_t lastBlock[BLOCKSIZE];
//     uint32_t vaildlen = length;
...
//                 // PrintDataLE(lastBlock, 16 - padding);
//             }
//         }
//     }
//     delete[] S_IV;
...
void decrypt(uint32_t length, uint32_t *word, uint8_t *IV)
{
    ...
    for (int i = 0; i < times; i++)
    {
        ...
        if (i == times - 1)
        {
            uint8_t padding = tmp[15]; // 获取填充字节数
            if (padding > 0 && padding <= BLOCKSIZE)
            {
                validLen -= padding; // 更新有效数据长度
                out.write((char *)tmp, BLOCKSIZE - padding);
                // PrintDataLE(tmp, BLOCKSIZE - padding);
            }
        }
        else
        {
...
int main()
{
    ...
    // uint8_t key[16] = {0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c}; // 密钥
    uint8_t key[16];
